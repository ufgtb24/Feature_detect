...
from multiTask import CrossStitchLayer, BALANCED, IMBALANCED
...
def inception_v3_base(input,
    ...
    with tf.variable_scope(scope):
        ...
        with arg_scope(
                ...
                padding='SAME'):
            # 128 x 128 x 1
            layer_scope = 'Conv2d_1_'
            nets=[input]*task_num

            nets = [layers.conv3d(input, depth(16), 3, stride=2, scope=layer_scope + str(i)) for i, input in
                    enumerate(nets)]
            # 64 x 64 x 16
            layer_scope = 'Conv2d_2_'
            nets = [layers.conv3d(net, depth(32), 3, scope=layer_scope + str(i)) for i, net in enumerate(nets)]
            
            # 64 x 64 x 32
            layer_scope = 'MaxPool_2_'
            nets = [layers_lib.max_pool3d(net, 2, stride=2, scope=layer_scope + str(i)) for i, net in enumerate(nets)]
            csl = CrossStitchLayer(task_num, num_subspaces, init_scheme)
            nets = csl.stitch(nets,[None,32,32,32,32])

            # 32 x 32 x 32.
            # increase dimension
            layer_scope = 'Conv2d_3_'
            nets = [layers.conv3d(net, depth(40), 1, scope=layer_scope + str(i)) for i, net in enumerate(nets)]
            
            # 32 x 32 x 40.
            layer_scope = 'Conv2d_4_'
            nets = [layers.conv3d(net, depth(96), 3, scope=layer_scope + str(i)) for i, net in enumerate(nets)]
            
            # 32 x 32 x 96.
            layer_scope = 'MaxPool_4_'
            nets = [layers_lib.max_pool3d(net, 2, stride=2, scope=layer_scope + str(i)) for i, net in enumerate(nets)]
            csl = CrossStitchLayer(task_num, num_subspaces, init_scheme)
            nets = csl.stitch(nets,[None,16,16,16,96])
        ...
        with arg_scope(
                ...
                padding='SAME'):
            # mixed: 16 x 16 x 128.
            layer_scope = 'Mixed_5a_'
            next_nets = []
            for i, net in enumerate(nets):
                with variable_scope.variable_scope(layer_scope + str(i)):
            ...
            nets = next_nets
            csl = CrossStitchLayer(task_num, num_subspaces, init_scheme)
            nets = csl.stitch(nets)
            
            # mixed4: 8 x 8 x 384.
            layer_scope = 'Mixed_6b_'
            next_nets = []
            for i, net in enumerate(nets):
                with variable_scope.variable_scope(layer_scope + str(i)):
                    with variable_scope.variable_scope('Branch_0'):
...
def inception_v3(inputs,
    ...
    output_dim=[3*len(content['feature_need']) for content in task_dict.values()]

    with tf.variable_scope(
            scope, reuse=reuse) as scope:
        with arg_scope(
                [bn_layer_top, layers_lib.dropout], is_training=is_training):
            nets = inception_v3_base(
                inputs,
                scope='Base',
                min_depth=min_depth,
            ...
            layer_scope = 'Final_Pool_'
            next_nets = []
            for i, net in enumerate(nets):
                with tf.variable_scope(layer_scope + str(i)):
            ...
            for i, net in enumerate(nets):
                with tf.variable_scope(layer_scope + str(i)):
                    net = layers_lib.dropout(
                        net, keep_prob=0.5, scope='dropout')
                    # 1024
                    ...
                    logit = layers.conv3d(
                        ...
                        scope='conv')
                    # 6
                    logits.append(array_ops.squeeze(logit, [1, 2, 3], name='output'))
