...
#
model_name = "F:\\ProjectData\\tmp\\model\\low5104_3\\20181013-1036\\low_graph.pb"
...
with tf.gfile.FastGFile(os.path.join("./", model_name), 'rb') as f:
    ...
    graph = tf.import_graph_def(graph_def, name='') # not use
...
x = test_batch_gen.get_batch()['box']
...
shape_dict = {'import/detector/input_box:0': [1,128,128,128]}
dtype_dict = {'import/detector/input_box:0': 'uint8'}
...
# set inputs
m.set_input('import/detector/input_box:0', tvm.nd.array(x.astype(dtype)))
m.set_input(**params)
# execute
m.run()
# get outputs
tvm_output = m.get_output(0, tvm.nd.empty(((1, 27)), 'float32'))
...
def run_inference_on_image(image):
    ...
    with tf.Session() as sess:
        ...
        predictions = sess.run(softmax_tensor,
                               {'import/detector/input_box:0': image_data})
