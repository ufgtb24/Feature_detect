        ...
        with tf.variable_scope(scope):
            ...
            with slim.arg_scope(icp.inception_v3_arg_scope()):
                ...
                with tf.variable_scope('error'):
                    ...
                    self.loss_summary = tf.summary.scalar('loss', self.error)
            if need_optim:
...
##################################
    ...
    with tf.Session(config=config) as sess:
        ...
        for data_set, config_content in DATA_DICT.items():
            ...
            file_writer = tf.summary.FileWriter(os.path.join( 'log',data_set), tf.get_default_graph())
            ...
            for iter in range(TOTAL_EPHOC):
                ...
                if iter % test_step == 0:
                    ...
                    loss_test, pred_get, loss_summary_get = sess.run([detector.error, detector.pred, detector.loss_summary], feed_dict=feed_dict)
                    ...
                    if iter%(test_step*10)==0:
                        file_writer.add_summary(loss_summary_get,global_step=test_step*10)
