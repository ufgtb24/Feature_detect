...
def passthrough_layer(lowRes, highRes, kernel, depth, size, name):
    with tf.variable_scope(name):
        ...
        y = tf.concat([lowRes, highRes], axis=1)
        return y
...
def block16(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'Block16', [net], reuse=reuse):
        ...
        mixed = tf.concat(axis=1, values=[tower_conv, tower_conv1, tower_conv2])
        up = slim.conv3d(mixed, net.get_shape()[1], 1, normalizer_fn=None,
    ...
    with tf.variable_scope(scope, 'Block8', [net], reuse=reuse):
        ...
        with tf.variable_scope('Branch_1'):
            ...
            tower_conv1_3 = slim.conv3d(tower_conv1_2, 128, [3, 1, 1],
                                        scope='Conv2d_0c_3')
        mixed = tf.concat(axis=1, values=[tower_conv, tower_conv1_3])
        up = slim.conv3d(mixed, net.get_shape()[1], 1, normalizer_fn=None,
                         activation_fn=None, scope='Conv2d_1x1')
...
def inception_resnet_v2_base(inputs,
    ...
    with tf.variable_scope(scope, 'InceptionResnetV2', [inputs]):
        with slim.arg_scope([slim.conv3d, slim.max_pool3d, slim.avg_pool3d],
                            stride=1, padding='SAME',data_format="NCDHW"):
            # input is 128 1
            # 64 16
